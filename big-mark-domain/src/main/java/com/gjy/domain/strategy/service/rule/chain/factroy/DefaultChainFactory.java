package com.gjy.domain.strategy.service.rule.chain.factroy;
...
public class DefaultChainFactory {
    ...
    public ILogicChain openLogicChain(Long strategyId) {
        ...
        for (int i = 1; i < ruleModels.length; i++) {
            ILogicChain iLogicChain = logicChainGroup.get(ruleModels[i]);
            current = current.appendNext(iLogicChain);
        }
        // 责任链的最后装填默认责任链
